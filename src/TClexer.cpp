        ...
        while (!tokenFound) {
            ...
            if (charBuff == EOFCHAR) {
                    ...
                    switch (charBuff) {                                         // Testing the lookahead buffer
                        case '.':
                            if (DECIMALS > 0) {                                 // There has been a '.' in current token
                                ...
                                DECIMALS++;
                                lexeme += charBuff;                 // Update counter & load lookahead
                ...
                if (tokenChecker(lexeme, "WRITE")) {
                    ...
                    if (!BLOCK_COMMENT) { if (!BLOCK_COMMENT) { tokenFound = true; }}
                }
...
//                            reportDEBUG("  ", "Comments", "End Clearing Lexeme" + lexeme);
                                commentDepth--;
                                if (commentDepth != 0) {
                                    reportDEBUG("  ", "depth", std::to_string(commentDepth));
                                }
                                lexeme = "";
                                charBuff = getChar();
//                            break;
...
//                                reportDEBUG("  ", "Comments", "Begging Clearing Lexeme");
                ...
                if (commentDepth == 0) { BLOCK_COMMENT = false; }
                else { BLOCK_COMMENT = true; }
            }
        }
...
//					reportWARNING("  ", " Scanner: '.' Illegal Character. BLOCK_COMMENTing"); pos++;
//				}
//			}
//			if (ch == '\0') { line = getNextLine(); ch = line[pos]; }
//			if (isInAlphabet(ch) || isspace(ch))  { break; }
//			reportWARNING("  ", " Scanner: \'" + holder + "\' Illegal Character. BLOCK_COMMENTing"); pos++;
//			break;
//		} while(true);
/// try 2
        while (!infile.eof()) {
            ...
            if ((ch == '.')) {
                if (isspace(line[pos + 1])) {
                    reportWARNING("  ", " Scanner: '.' Illegal Character. BLOCK_COMMENTing");
                    pos++;
                }
            }
            if (ch == '\0') {
            ...
            if (isInAlphabet(ch) || isspace(ch)) { break; }
                ...
                reportWARNING("  ", " Scanner: \'" + holder + "\' Illegal Character. BLOCK_COMMENTing");
            }
            //return EOFCHAR;
            //break;
        }
    ...
    string getNextLine() {
        ...
        if (verbose) reportDEBUG(" ", "INPUT", " " + num + ": " + line + comment);
        return line;
    }
