...
namespace toyc {
    ...
    TCtoken *TClexer::getToken() {
        ...
        while (isspace(charBuff) && (charBuff != EOFCHAR))
            charBuff = getChar();    // Ignore space charachters that are not at the end of the file
        if (charBuff ==
            EOFCHAR) {                                            // If it is the EOF, report that and return an EOFILE token
            ...
            return t;
                ...
                charBuff))                                            // TODO: add error handling for cases with more than one consecutive '.' or 'E'
        {                                                                    // Code for detecting NUMBER Tokens CD
            ...
            else if (tokenChecker(lexeme, "THEN"))      t = new TCtoken(THEN, lexeme);
            else if (tokenChecker(lexeme, "GOTO"))      t = new TCtoken(GOTO, lexeme);
            else if (tokenChecker(lexeme, "SKIP"))      t = new TCtoken(SKIP, lexeme);
            else if (tokenChecker(lexeme, "AND"))       t = new TCtoken(AND, lexeme);
            else if (tokenChecker(lexeme, "OR"))        t = new TCtoken(OR, lexeme);
            else if (tokenChecker(lexeme, "DO"))        t = new TCtoken(DO, lexeme);
            else if (tokenChecker(lexeme, "FOR"))       t = new TCtoken(FOR, lexeme);
            else if (tokenChecker(lexeme, "INT"))       t = new TCtoken(INT, lexeme);
            else if (tokenChecker(lexeme, "ELSE"))      t = new TCtoken(ELSE, lexeme);
            else if (tokenChecker(lexeme, "CHAR"))      t = new TCtoken(CHAR, lexeme);
            else if (tokenChecker(lexeme, "CASE"))      t = new TCtoken(CASE, lexeme);
            else if (tokenChecker(lexeme, "WHILE"))     t = new TCtoken(WHILE, lexeme);
            else if (tokenChecker(lexeme, "SWITCH"))    t = new TCtoken(SWITCH, lexeme);
            else if (tokenChecker(lexeme, "RETURN"))    t = new TCtoken(RETURN, lexeme);
            else if (tokenChecker(lexeme, "BREAK"))     t = new TCtoken(BREAK, lexeme);
            else if (tokenChecker(lexeme, "DEFAULT"))   t = new TCtoken(DEFAULT, lexeme);
            else if (tokenChecker(lexeme, "CONTINUE"))  t = new TCtoken(CONTINUE, lexeme);
            else if (tokenChecker(lexeme, "STRING"))    t = new TCtoken(STRING, lexeme);
            else if (tokenChecker(lexeme, "SKIP"))      t = new TCtoken(SKIP, lexeme);
            else if (tokenChecker(lexeme, "NONE"))      t = new TCtoken(NONE, lexeme);
            else if (tokenChecker(lexeme, "THEN"))      t = new TCtoken(THEN, lexeme);
            else { t = new TCtoken(ID, lexeme); }
        } else {
            lexeme += charBuff;                                                // TODO: Fix this properly by calling GetToken() recursively
            bool failandor = true;                                            // this boolean is here for the patch I added to allow the
            ...
            {                                                                // The way it works is that if there is only
                case '|':                                                    // one of those characters, then it fails over to the rest of the
                    ...
                    if (charBuff ==
                        '|')                                    // and it doesn't work in cases where there is a '|' immediately
                    {                                                        // followed by a '&'. However, it should be good enough for now. CD
                        ...
                        failandor = false;
                        break;
                    }                                                        // Checks for two | consecutively CD
                    else {
                    ...
                    break;
                case '&':                                                    // Checks for two & Consecutively CD
                    charBuff = getChar();
                    if (charBuff == '&') {
                        lexeme += charBuff;
                ...
                switch (charBuff) {                                             // Largely unchaged; Added other symbols DM
                    ...
                    case '/':                                                   // New symbol DM
                        ...
                        if (charBuff ==
                            ...
                            t = new TCtoken(DIVOP,
                                            lexeme);                     // If not Comment or Block Comment, DIVOP
                            charBuff = getChar();
                            break;
                        }
                        break;
